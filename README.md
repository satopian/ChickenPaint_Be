...
### テスト用の開発サーバを起動
`make dev`で開発サーバが起動します。   
ブラウザで開発サーバのURlを開いてChickenPaint Beの動作確認を行う事ができます。  
`dist/`にはディレクトリに開発サーバで使用するファイル一式が入ります。      
`dist/`ディレクトリにビルドされるファイルは開発サーバを使った動作確認にしか使えないため、`dist/`ディレクトリ内のファイルは配布には適していません。  
