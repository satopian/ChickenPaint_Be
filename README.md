...
### 何枚レイヤーが存在してもワンタップで表示を左右反転
- レイヤーが50枚存在する画像の表示を反転するために、50枚のレイヤーを左右反転して、さらにもとに戻すために50枚のレイヤーを左右反転するといった無駄な作業が発生する事がありました。
そのため、｢表示の左右反転｣機能を追加し、レイヤーの枚数が何枚あっても表示の左右反転ができるようにしました。
この操作は実際の画像は変更されないため、アンドゥの履歴にも残りません。
