...
#### ｢全レイヤー混合｣から｢下のレイヤーと混色｣へ
- レイヤーパレットの｢全レイヤー混合｣の動作を変更し、｢下のレイヤーと混色｣にラベルを変更しました。  
これまでは、｢全レイヤー混合｣のチェックボックスにチェックした時に全レイヤーを統合した画像の色と混色していました。    
しかし、線画の線が黒の時にこの機能を使用すると線画の黒も混ざってしまいます。  
そのため、実質線画の彩色の時には｢全レイヤー混合｣機能を使う事ができませんでした。  
そのため、色が混ざる範囲を、現在のレイヤーとその下のレイヤーに限定して、現在のレイヤーより上のレイヤーを混色の対象から除外する新しい機能が必要になりました。  
それが｢下のレイヤーと混色｣機能です。  
現在のレイヤーより上のレイヤーの色を拾わないため、線画に彩色する時に問題なく機能するようになります。  
例えば顔の頬の赤みだけ上のレイヤーに描きたい、しかし、下のレイヤーの肌色となじませたいといった時などに効果的に使う事ができるようになりました。  
混色系の水彩ブラシや指先ツールなど色を混ぜながら描画するブラシの選択時にのみこのオプションが有効になります。    
  
<img width="239" height="379" alt="image" src="https://github.com/user-attachments/assets/aa0b4b9f-7ad6-40ee-8e34-3fabd158b3d2" />
