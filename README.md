...
｢litaChix｣は、ノルウェー語で小さいを意味する｢lita｣とひよこの｢Chix｣を組み合わせた造語です。   
...
[お絵かき掲示板のペイントツールlitaChixの新機能でミュイ・フレイア - YouTube](https://www.youtube.com/watch?v=Lcw08qJiJ88)

### カラーハーフトーンフィルタと単色ハーフトーンフィルタを追加
- カラーをCMYのドットで表現する｢カラーハーフトーン｣フィルタを追加しました。
- 現在の色で単色ハーフトーンを作成する｢単色ハーフトーン｣フィルタを追加しました。

[お絵かき掲示板のペイントツールlitaChix(リタチックス)の新機能 - YouTube](https://www.youtube.com/watch?v=YMBq-F160Oo)
...
- 「未保存のデータがあります。復元しますか？」という確認ダイアログで｢OK｣を選択すると復元します。  
...
- SHIFT+Mでマスク削除
- CTRL+Iでネガポジ反転
- SHIFT+ドラッグでブラシサイズと不透明度のスライダーを緩やかに調整
- CTRL+スペース+ドラッグまたは、Z+ドラッグでズーム
(左にドラッグで縮小し右にドラッグで拡大)   
- CTRL+ALT+ドラッグでブラシサイズ変更
(左にドラッグで縮小し右にドラッグで拡大)  
...
### スポイトを改善
- スポイトを改善しました。色を取得する対象を｢表示色から取得｣｢レイヤーから取得｣から選択できるようになりました。
また取得した色を円カーソルに表示するようになりました。
  
<img width="214" height="264" alt="Screen-2026-03-29_11-59-21" src="https://github.com/user-attachments/assets/083dfa9d-5fee-40ec-b641-bede784b0fa3" />
  
### Bz曲線を改善
- Bz曲線を改善しました。2番目のハンドルが表示された瞬間にBzプレビューの曲線が大幅に移動していましたが、今は1番目のハンドルの操作時に2番目のハンドルの開始位置が決定ずみです。
そのためBzプレビューの曲線は最初から意図した位置に表示されます。    
また、エスケープキー押下、マウス右クリックでBzの処理をキャンセルできるようになりました。
...
投稿が完了して画面が移動する直前に、`window.handleExit()`関数を実行します。  
