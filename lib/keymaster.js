...
function dispatch(event) {
  ...
  if (!assignKey.filter.call(document, event)) return;
...
// 利用側の「key.isPressed」などの形式を維持するため、
// 関数オブジェクトに必要なメソッドをプロパティとして追加してエクスポートする
// クライアントAPI（プロパティとメソッド）をバインド
...
// 修飾キーのプロパティ
assignKey.shift = false;
assignKey.alt = false;
assignKey.option = false;
assignKey.ctrl = false;
assignKey.control = false;
assignKey.command = false;

// 後方互換性およびダミーとしてのnoConflict
assignKey.noConflict = function () {
  return assignKey;
};

/**
 * @typedef {Object} KeymasterModifiers
 ...
 *
 * @property {typeof setScope} setScope
 * @property {typeof getScope} getScope
 * @property {typeof deleteScope} deleteScope
 * @property {typeof filter} filter
 * @property {typeof isPressed} isPressed
 * @property {typeof getPressedKeyCodes} getPressedKeyCodes
 * @property {typeof unbindKey} unbind
 * @property {() => any} noConflict
 */

/** @type {typeof assignKey & KeymasterModifiers} */
const exportedKeymaster = /** @type {any} */ (assignKey);

// ESモジュールとしてエクスポート
export default exportedKeymaster;
