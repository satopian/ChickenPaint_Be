...
const IMAGE_DIMENSION = 256,
  [translucentRandomImageBottom, translucentRandomImageTop] =
    TestUtil.generateRandomImagePairForBlendTest(
      IMAGE_DIMENSION,
      IMAGE_DIMENSION,
      false,
      false,
  ...
  [transparentRandomImageBottom, transparentRandomImageTop] =
    TestUtil.generateRandomImagePairForBlendTest(
      IMAGE_DIMENSION,
      IMAGE_DIMENSION,
      0,
  ...
  opaqueRandomImageBottom = TestUtil.generateRandomImageForBlendTest(
    IMAGE_DIMENSION,
    IMAGE_DIMENSION,
    255,
    1283714,
...
function generateRandomMask(width, height, seed) {
  const mask = new CPGreyBmp(width, height, 8),
 ...
 */
function testPassthroughBlendingOperation(
  fusionHasTransparency,
  imageAlpha,
  masked,
) {
  const fuse = function (bottom, top, mask) {
    bottom = bottom.clone();
  ...
  if (imageAlpha === 0) {
    it("equals the bottom layer if layer alpha==0", function () {
      let referenceImage = fusionHasTransparency
        ? translucentRandomImageBottom
        : opaqueRandomImageBottom;

      assert(
        TestUtil.bitmapsAreEqual(
          fuse(
            referenceImage,
  ...
  if (!fusionHasTransparency) {
    it("gives the same result using both opaque and transparent fusion routines if bottom pixels are effectively opaque", function () {
      let opaqueFusionResult, transparentFusionResult;
      ...
      assert(
        TestUtil.bitmapsAreEqual(opaqueFusionResult, transparentFusionResult),
      );
    });
  }
}
...
/**
 *
 * @param {Number} layerMode
 * @param {boolean} fusionHasTransparency
 * @param {Number} imageAlpha
 * @param {boolean} masked
 */
function testRegularBlendingOperation(
  layerMode,
  fusionHasTransparency,
  imageAlpha,
  masked,
  ...
  it(
    "does not modify the bottom layer if the top layer (alpha " +
      imageAlpha +
      ") has effectively transparent pixels",
    function () {
      let referenceImage = fusionHasTransparency
        ? translucentRandomImageBottom
        : opaqueRandomImageBottom;
  ...
  if (masked && imageAlpha !== 0) {
    it(
      ...
      function () {
        ...
        assert(
          TestUtil.bitmapsAreEqual(
            fuse(referenceImage, translucentRandomImageTop, blackMask),
            referenceImage,
          ),
        );
      },
  ...
  if (!masked) {
    it(
      "gives the same result when mask is white as when mask is absent (alpha " +
        imageAlpha +
        ")",
      function () {
        let whiteMaskResult, noMaskResult;
        ...
        assert(TestUtil.bitmapsAreEqual(whiteMaskResult, noMaskResult));
      },
    );
  }
  ...
  if (!fusionHasTransparency && imageAlpha !== 0) {
    it("gives the same result using both opaque and transparent fusion routines if bottom pixels are effectively opaque", function () {
      ...
      let result = TestUtil.bitmapsAreEqual(
        opaqueFusionResult,
...
describe("CPBlend", function () {
  for (
    let layerMode = CPBlend.LM_FIRST;
    layerMode <= CPBlend.LM_LAST;
    layerMode++
  ) {
    for (let fusionHasTransparency of [false, true]) {
      for (let topLayerOpaque of [false, true]) {
        for (let masked of [false, true]) {
          describe(
            "#" +
              CPBlend.blendFunctionNameForParameters(
                fusionHasTransparency,
                topLayerOpaque ? 100 : 50,
                layerMode,
                masked,
              ),
            function () {
              for (let imageAlpha of topLayerOpaque ? [100] : [23, 0]) {
                if (layerMode === CPBlend.LM_PASSTHROUGH) {
                  testPassthroughBlendingOperation(
                    fusionHasTransparency,
                    imageAlpha,
                    masked,
                  );
                } else {
                  testRegularBlendingOperation(
                    layerMode,
                    fusionHasTransparency,
                    imageAlpha,
                    masked,
