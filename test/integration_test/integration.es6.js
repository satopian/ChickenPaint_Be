 ...
 *
 * @param {Number} max - Maximum integer (exclusive) to generate
 * @returns {Number}
 */
...
let pageURL = new URL(window.location),
  randomSeed = pageURL.searchParams.get("seed")
    ? parseInt(pageURL.searchParams.get("seed"), 10) | 0
    : Random.int32(Random.engines.browserCrypto),
...
function saveImage(image, name) {
  const flat = binaryStringToByteArray(image.getAsPNG(0)),
...
class Step {
  ...
  perform(chickenPaint) {
    if (this.action) {
      ...
      const artwork = chickenPaint.getArtwork();
      ...
      switch (this.parameters.task) {
        case "paint":
          ...
          if (this.parameters.pointIndex === 0) {
            if (
              !artwork.beginStroke(
                this.parameters.points[0].x,
                this.parameters.points[0].y,
                1.0,
              )
            ) {
              return true;
            ...
            artwork.continueStroke(
              this.parameters.points[this.parameters.pointIndex].x,
              this.parameters.points[this.parameters.pointIndex].y,
              1.0,
            );
          }
        ...
        case "history":
          ...
          let action = this.parameters.actions[this.parameters.actionIndex];
          ...
          if (chickenPaint.isActionAllowed(action)) {
            ...
            if (this.parameters.historyDepth >= 0) {
              const currentImage = artwork.fusionLayers();
              ...
              if (this.parameters.history[this.parameters.historyDepth]) {
                ...
                if (
                  !currentImage.equals(
                    this.parameters.history[this.parameters.historyDepth],
                  )
                ) {
                  ...
                  throw new Error(
                    "Failure for undo/redo to restore image correctly",
                  );
                }
              } else {
        ...
        case "relocateLayer":
          let layers = artwork.getLayersRoot().getLinearizedLayerList(false);
        ...
        case "setActiveLayer":
          chickenPaint.actionPerformed({
            ...
            layer: artwork.getLayersRoot().getLinearizedLayerList(false)[
              this.parameters.layerIndex
            ],
        ...
        case "boxBlur":
          artwork.boxBlur(
            this.parameters.radiusX,
            this.parameters.radiusY,
...
class RandomTest extends Test {
   ...
   */
  generateStep() {
    const chickenPaint = this.chickenPaint,
      artwork = chickenPaint.getArtwork(),
      stepGenerators = [
        function () {
          return null;

          let action = pick(["CPZoom100", "CPZoomIn", "CPZoomOut"]);
        ...
        function () {
          ...
          switch (action) {
            case "CPFloodFill":
              let x = Random.real(-50, artwork.width + 50)(randomEngine),
        ...
        function () {
          switch (pick(["setColor", "setBrushSize"])) {
            case "setColor":
              return new Step(null, {
                task: "setColor",
                color: Random.int32(randomEngine),
              });
        ...
        function () {
          const x = Random.real(-50, artwork.width + 50)(randomEngine),
            y = Random.real(-50, artwork.height + 50)(randomEngine),
        ...
        function () {
          const selectionCenter = new CPPolygon(
              artwork.getSelectionAutoSelect().toPoints(),
            ).getCenter(),
          ...
          transform.translate(
            Random.real(-150, 150)(randomEngine),
            Random.real(-150, 150)(randomEngine),
          );
          transform.rotateAroundPoint(
            Random.real(-2 * Math.PI, 2 * Math.PI)(randomEngine),
            selectionCenter.x,
            selectionCenter.y,
          );
        ...
        function () {
          ...
          switch (action) {
            case "CPSetLayerLockAlpha":
              return new Step(
                { action: action, lock: Random.bool(randomEngine) },
                {},
              );

            case "CPSetLayerAlpha":
              let alphaMode = randInt(3),
                alpha;
            ...
            case "CPSetLayerBlendMode":
              return new Step(
                { action: action, blendMode: randInt(CPBlend.LM_LAST + 1) },
                {},
              );
          }
        },
        function () {
          ...
          if (
            ((action == "CPAddLayer" || action == "CPAddGroup") &&
              layers.length > 5) ||
...
function stepFromJSON(json) {
  return new Step(
    json.action ? clone(json.action) : null,
    json.parameters ? clone(json.parameters) : null,
  );
}
...
function minifyTestcase(steps) {
  ...
  let minPrefixLength = 0,
    ...
    attempt = function () {
      ...
      let chickenPaint = new ChickenPaint({});

      if (minPrefixLength < maxPrefixLength) {
        mid = Math.ceil((minPrefixLength + maxPrefixLength) / 2);

        let prefixSteps = steps.slice(0, mid).map(stepFromJSON),
        ...
        try {
          ...
          throw (
            "We received an error when evaluating a prefix of the operations, this shouldn't happen. " +
            e
          );
        }
        ...
        return chiSave(chickenPaint.getArtwork())
          ...
          .then(
            (artwork) => {
              try {
                let newChickenPaint = new ChickenPaint({
                  artwork: artwork,
                });
        ...
        let prefixSteps = steps.slice(0, minPrefixLength).map(stepFromJSON),
        ...
        return chiSave(chickenPaint.getArtwork()).then((chibiResult) => {
          console.log(
            "Testcase reduced from " +
              steps.length +
              " steps to " +
              postfixSteps.length +
              " steps, to be applied to saved drawing testcase.chi",
          );
          FileSaver.saveAs(chibiResult.bytes, "testcase.chi");
          FileSaver.saveAs(
            new Blob([JSON.stringify(postfixSteps)], {
              type: "text/plain;charset=utf-8",
            }),
            "testcase.json",
          );
          console.log(
            "Exception generated by the last step was ",
            lastException,
          );
        });
      }
...
export default function IntegrationTest() {
  ...
  function continueTest() {
    if (test.done()) {
      ...
      let wasPaintOperation =
        step.parameters && step.parameters.task == "paint";
