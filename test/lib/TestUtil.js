...
TestUtil.bitmapMaxDifference = function (bitmap1, bitmap2) {
  ...
  for (
    let pixIndex = 0;
    pixIndex < bitmap1.data.length;
    pixIndex += CPColorBmp.BYTES_PER_PIXEL
  ) {
    for (
      let i = CPColorBmp.RED_BYTE_OFFSET;
      i <= CPColorBmp.GREEN_BYTE_OFFSET;
      i++
    ) {
      let difference = Math.abs(
        bitmap1.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] *
          bitmap1.data[pixIndex + i] -
          bitmap2.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] *
            bitmap2.data[pixIndex + i],
      );
...
const COLOR_BITMAP_STRING_COLORS = {
...
TestUtil.greyBitmapAsString = function (bitmap) {
  ...
  for (let y = 0; y < bitmap.height; y++) {
    for (let x = 0; x < bitmap.width; x++, pixIndex++) {
      let color = bitmap.data[pixIndex],
        found = false;
...
TestUtil.colorBitmapFromString = function (text) {
  ...
  for (let y = 0; y < imageHeight; y++) {
    ...
    for (
      ...
      let color = COLOR_BITMAP_STRING_COLORS[line.charAt(x)];
      ...
      image.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] =
        (color >> 24) & 0xff;
    }
  }
...
TestUtil.testLayerPaintOperation = function (test) {
  var beforeImage =
      typeof test.beforeImage == "string"
        ? TestUtil.colorBitmapFromString(test.beforeImage)
        : test.beforeImage,
    beforeMask =
      typeof test.beforeMask == "string"
        ? TestUtil.greyBitmapFromString(test.beforeMask)
        : test.beforeMask,
    expectImage =
      typeof test.expectImage == "string"
        ? TestUtil.colorBitmapFromString(test.expectImage)
        : test.expectImage,
    expectMask =
      typeof test.expectMask == "string"
        ? TestUtil.greyBitmapFromString(test.expectMask)
        : test.expectMask,
 ...
 * @param {Number} width
 * @param {Number} height
 * @param {Number|boolean} setAlpha - The alpha to set in the pixels, or false to generate a random alpha.
 * @param {Number} seed
 * @returns {CPColorBmp}
 */
TestUtil.generateRandomImageForBlendTest = function (
  width,
  height,
  setAlpha,
  seed,
) {
  const pixelChannelEdgeCases = [0, 1, 2, 127, 128, 129, 253, 254, 255],
  ...
  for (; pixIndex < image.data.length; pixIndex += CPColorBmp.BYTES_PER_PIXEL) {
    image.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] =
      setAlpha === false ? randomByte(randomEngine) : setAlpha;
    image.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] =
      randomByte(randomEngine);
    image.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] =
      randomByte(randomEngine);
    image.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] =
      randomByte(randomEngine);
  }
 ...
 * @param {Number} width
 * @param {Number} height
 * @param {Number|boolean} setAlphaBottom - Pass an integer to force the alpha in this image, or false to use random values
 * @param {Number|boolean} setAlphaTop - Pass an integer to force the alpha in this image, or false to use random values
 * @param {Number} seed
 * @returns {[CPColorBmp,CPColorBmp]} - An array [bottomImage, topImage]
 */
TestUtil.generateRandomImagePairForBlendTest = function (
  width,
  height,
  setAlphaBottom,
  setAlphaTop,
  ...
  for (let bottomColor of pixelChannelEdgeCases) {
    for (let bottomAlpha of setAlphaBottom === false
      ? pixelChannelEdgeCases
      : [setAlphaBottom]) {
      for (let topColor of pixelChannelEdgeCases) {
        for (let topAlpha of setAlphaTop === false
          ? pixelChannelEdgeCases
          : [setAlphaTop]) {
          /*
           ...
           */
          bottomImage.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] =
            bottomAlpha;
          bottomImage.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] = bottomColor;
          bottomImage.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] =
            bottomColor;
          bottomImage.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] =
            bottomColor;
  ...
  for (
    ;
    pixIndex < bottomImage.data.length;
    pixIndex += CPColorBmp.BYTES_PER_PIXEL
  ) {
    bottomImage.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] =
      setAlphaBottom === false ? randomByte(randomEngine) : setAlphaBottom;
    bottomImage.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] =
      randomByte(randomEngine);
    bottomImage.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] =
      randomByte(randomEngine);
    bottomImage.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] =
    ...
    topImage.data[pixIndex + CPColorBmp.ALPHA_BYTE_OFFSET] =
      setAlphaTop === false ? randomByte(randomEngine) : setAlphaTop;
    topImage.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] =
      randomByte(randomEngine);
    topImage.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] =
      randomByte(randomEngine);
    topImage.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] =
      randomByte(randomEngine);
  }
