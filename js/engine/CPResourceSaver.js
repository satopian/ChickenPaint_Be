  ...
  function reportProgress(progress) {
    if (progress === null) {
      ...
      that.emitEvent("savingProgress", [
        progress,
        _("Saving drawing...") + " (" + Math.round(progress * 100) + "%)",
      ]);
    }
  }
  ...
  async function postDrawing(formData) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          _("The total size of FormData is within the acceptable range."),
        );
      }
    } catch (error) {
  ...
  this.save = function (save_options = {}) {
    ...
    serializeLayers
      .then(function (chibiResult) {
        ...
        if (options.url) {
          let marker = "This marker ensures the upload wasn't truncated",
          ...
          if (zip) {
            saveFilesAsZip(saveFilename, flatBlob, chibiResult, swatchesBlob);
          } else if (savedb) {
