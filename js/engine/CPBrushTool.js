...
import CPBrushDab from "./CPBrushManager.js";
...
/**
 * @param {typeof CPColorFloat[]} array
 * @returns {typeof CPColorFloat}
 */
...
export class CPBrushTool {
  /**
   * @param {typeof CPGreyBmp} strokeBuffer - A 32-bit image we can use for buffering paint operations to be merged later.
   * @param {typeof CPRect} strokedRegion - The area of the strokeBuffer we've painted on.
   */
  ...
  /**
   * @param {typeof CPColorBmp|typeof CPGreyBmp} destImage - Image to paint to (for those brushes not using the strokeBuffer)
   * @param {typeof CPRect} imageRect - The area on the canvas that will be painted to
   * @param {typeof CPBrushInfo} brushConfig - The current brush tip configuration
   * @param {typeof CPRect} brushRect - The rectangle from the dab which will be painted to the canvas
   * @param {typeof CPBrushDab} dab
   * @param {typeof CPColorBmp|typeof CPGreyBmp} sampleImage - Image to sample from (either the current layer/mask or the fusion depending on user's
   ...
   *
   * @param {typeof CPColorBmp} destImage - The layer to draw onto
   * @param {typeof CPColorBmp} undoImage - The original pixels for the layer before the stroke began
  ...
  mergeOntoImage(destImage, undoImage, color) {
    ...
    for (
      ...
      for (
        ...
        if (strokeAlpha > 0) {
          let destAlpha = undoData[dstOffset + CPColorBmp.ALPHA_BYTE_OFFSET],
            newLayerAlpha =
              (strokeAlpha + (destAlpha * (255 - strokeAlpha)) / 255) | 0,
   ...
   *
   * @param {typeof CPGreyBmp} destMask - The destination to write to
   * @param {typeof CPGreyBmp} undoMask - The original contents of the mask
      ...
      for (let x = 0; x < width; x++, srcOffset++, dstOffset++) {
        ...
        if (strokeAlpha > 0) {
          ...
          destMaskData[dstOffset] =
            (color * strokeAlpha + undoMaskData[dstOffset] * invAlpha) / 255;
        }
      }
    }
  }
   ...
   *
   * @param {typeof CPRect} brushRect - Rectangle from brushShape array to paint
   * @param {typeof CPRect} imageRect - Rectangle of the destination image that corresponds to brushRect
        ...
        if (brushAlpha != 0) {
          strokeData[strokeOffset] = Math.min(
            ...
            strokeData[strokeOffset] +
              ((255 - strokeData[strokeOffset] / 255) * brushAlpha) / 255,
          );
        }
      }
    }
   ...
   *
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
      ...
      for (
        ...
        if (
          ...
          destImageData[dstOffset + CPColorBmp.RED_BYTE_OFFSET] =
            undoData[dstOffset + CPColorBmp.RED_BYTE_OFFSET] * scale;
          destImageData[dstOffset + CPColorBmp.GREEN_BYTE_OFFSET] =
            undoData[dstOffset + CPColorBmp.GREEN_BYTE_OFFSET] * scale;
          destImageData[dstOffset + CPColorBmp.BLUE_BYTE_OFFSET] =
            undoData[dstOffset + CPColorBmp.BLUE_BYTE_OFFSET] * scale;
        }
      }
    }
  }
        ...
        if (strokeAlpha > 0) {
          destMaskData[dstOffset] = Math.max(
            undoMaskData[dstOffset] -
              ((BURN_CONSTANT - undoMaskData[dstOffset]) * strokeAlpha) / 255,
            0,
          );
        }
      }
      ...
      for (
        ...
        if (strokeAlpha > 0) {
          let centralSampleWeight =
              (BLUR_MIN + ((BLUR_MAX - BLUR_MIN) * strokeAlpha) / 255) | 0,
          ...
          // x, y + 1
          addSample(
            y < undoImage.height - 1 ? destOffset + destYStride : destOffset,
          );
          // x - 1, y
          addSample(
            x > 0 ? destOffset - CPColorBmp.BYTES_PER_PIXEL : destOffset,
          );
          // x + 1, y
          addSample(
            x < undoImage.width - 1
    ...
    for (
      ...
      for (
        ...
        if (strokeAlpha > 0) {
          ...
          // x, y - 1
          sampleSum +=
            undoMaskData[y > 0 ? destOffset - destYStride : destOffset];
          // x, y + 1
          sampleSum +=
            undoMaskData[
              y < undoMask.height - 1 ? destOffset + destYStride : destOffset
            ];
          // x - 1, y
          sampleSum += undoMaskData[x > 0 ? destOffset - 1 : destOffset];
          // x + 1, y
          sampleSum +=
            undoMaskData[x < undoMask.width - 1 ? destOffset + 1 : destOffset];
    ...
    for (
      ...
      for (let x = 0; x < width; x++, srcOffset++, dstOffset++) {
        ...
        if (alpha1 > 0) {
          ...
          destMaskData[dstOffset] =
            (((color1 & 0xff) * alpha1 + undoMaskData[dstOffset] * invAlpha) /
              255) |
            0;
        }
      }
...
export class CPBrushToolWatercolor extends CPBrushToolDirectBrush {
   ...
   *
   * @param {typeof CPColorBmp} image - サンプリング対象の画像データ
   ...
   * @param {typeof CPColorFloat} brushColor - 全て透明だった場合に返される現在のブラシ色（fallback用）
   * @returns {typeof CPColorFloat} 算出された平均色、またはブラシ色
   */
...
export class CPBrushToolOil extends CPBrushToolDirectBrush {
   ...
   *
   * @param {typeof CPColorBmp} maskToSample
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
   ...
   * AG strokeBuffer.
   *
   * @param {typeof CPGreyBmp} destImage - Image that is being drawn onto
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
    ...
    for (
      ...
      for (
        ...
        let grey1 = brushData[bufferOffset],
          alpha1 =
            (((grey1 >> 8) * brushShape[bufferOffset] * alpha) / (255 * 255)) |
            0;
        ...
        if (alpha1 > 0) {
          ...
          strokeData[strokeOffset] =
            (255 << 8) |
            ((grey1 & 0xff) +
              (grey2 * invAlpha - (grey1 & 0xff) * invAlpha) / 255);
        }
      }
    }
  }
   ...
   *
   * @param {typeof CPColorBmp} imageToSample
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
  ...
  _accumulatePaintFromImage(imageToSample, brushRect, imageRect, alpha) {
    ...
    for (
      ...
      for (
        ...
        let alpha1 =
          ((sampleData[dstOffset + CPColorBmp.ALPHA_BYTE_OFFSET] * alpha) /
            255) |
          0;
   ...
   * 透明の中にある白や黒とはまざりません。
   *
   * @param {typeof CPColorBmp} destImage - 描画対象のレイヤー画像
   * @param {typeof CPRect} brushRect - 筆の矩形領域
   * @param {typeof CPRect} imageRect - 描画対象の矩形領域
      ...
      if (this._brushBuffer == null) {
        ...
        this._resaturateBrushWithColor(
          ...
          ~~(brushConfig.resat <= 0.0
            ? 0
            : Math.max(1, brushConfig.resat * brushConfig.resat * 255)),
          color & 0xffffff,
        );
        this._paintToColorStrokeBuffer(
          destImage,
...
export class CPBrushToolSmudge extends CPBrushToolDirectBrush {
   ...
   *
   * @param {typeof CPGreyBmp} sampleMask - Mask to sample from
   * @param {typeof CPRect} maskRect - Rectangle of the canvas that our brush covers
   * @param {typeof CPRect} brushRect - The corresponding rectangle within the brush buffer
    ...
    for (
      ...
      for (let x = 0; x < width; x++, brushOffset++, maskOffset++) {
        let sampleGrey = sampleMask.data[maskOffset],
          ...
          newBrushGrey =
            (sampleGrey * invAlpha + (oldBrushColor & 0xff) * alpha) / 255,
          newBrushColor =
   ...
   *
   * @param {typeof CPGreyBmp} destMask
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
   ...
   *
   * @param {typeof CPColorBmp} sampleImage - Image to sample from
   * @param {typeof CPRect} imageRect - Rectangle of the canvas that our brush covers
   * @param {typeof CPRect} brushRect - The corresponding rectangle within the brush buffer
   * @param {number} alpha - Alpha of brush (0-255)
   */
  _sampleFromImage(sampleImage, brushRect, imageRect, alpha) {
    ...
    for (
      ...
      for (
        ...
        let sampleRed =
            sampleImage.data[imageOffset + CPColorBmp.RED_BYTE_OFFSET],
          sampleGreen =
            sampleImage.data[imageOffset + CPColorBmp.GREEN_BYTE_OFFSET],
          sampleBlue =
            sampleImage.data[imageOffset + CPColorBmp.BLUE_BYTE_OFFSET],
          sampleAlpha =
            sampleImage.data[imageOffset + CPColorBmp.ALPHA_BYTE_OFFSET],
          oldBrushColor = brushData[brushOffset],
          newBrushColor =
            ((((sampleAlpha * invAlpha +
              ...
              24) &
              0xff000000) |
            ((((sampleRed * invAlpha + ((oldBrushColor >> 16) & 0xff) * alpha) /
              255) <<
              16) &
              0xff0000) |
            ((((sampleGreen * invAlpha +
              ...
              8) &
              0xff00) |
            (((sampleBlue * invAlpha + (oldBrushColor & 0xff) * alpha) / 255) &
              0xff);
   ...
   *
   * @param {typeof CPColorBmp} destImage
   * @param {typeof CPRect} brushRect
   * @param {typeof CPRect} imageRect
   * @param {Uint8Array} brushShape - Of the same width as this._brushBuffer
   */
    ...
    for (
      ...
      for (
        ...
        let paintColor = brushPaintData[srcOffset],
          strokeAlpha =
            (((paintColor >>> 24) * brushShape[srcOffset]) / 255) | 0;
