...
export default class CPColorBmp extends CPBitmap {
   ...
   *        正の値で下、負の値で上にずれる。
   */
  chromaticAberration(rect, offsetX, offsetY) {
   ...
   *        小さいほど密／大きいほど疎
   */
  colorHalftone(rect, dotSize, density = 1.0) {
   ...
   */
  edge(rect, edgeWidth, color, replaceWithInnerFill) {
    edgeWidth = Math.max(1, edgeWidth | 0);

    rect = this.getBounds ? this.getBounds().clipTo(rect) : rect;
   ...
   * @param {number} color - 0xRRGGBB
   */
  convertToDrawingColor(rect, color) {
    // 範囲をクランプ
    rect = this.getBounds().clipTo(rect);
   ...
   */
  fillWithNoise(rect, color = 0, opacity = 1) {
    rect = this.getBounds().clipTo(rect);
    ...
    for (var y = rect.top; y < rect.bottom; y++, pixIndex += yStride) {
      for (
        ...
        // 1. 各ピクセルごとに不透明度をランダムにする場合はここを Math.random() * opacity に。
        var currentSrcA = Math.random() * srcA;
    ...
    for (var y = rect.top; y < rect.bottom; y++, pixIndex += yStride) {
      ...
      for (var x = 0; x < w; x++, pixIndex += CPColorBmp.BYTES_PER_PIXEL) {
        ...
        if (newAlpha > 0) {
          ...
          this.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] =
            ((r * realAlpha +
              this.data[pixIndex + CPColorBmp.RED_BYTE_OFFSET] * invAlpha) /
              255) |
            0;
          this.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] =
            ((g * realAlpha +
              this.data[pixIndex + CPColorBmp.GREEN_BYTE_OFFSET] * invAlpha) /
              255) |
            0;
          this.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] =
            ((b * realAlpha +
              this.data[pixIndex + CPColorBmp.BLUE_BYTE_OFFSET] * invAlpha) /
              255) |
            0;
   ...
   */
  fillWithColorNoise(rect, opacity = 0.65) {
    rect = this.getBounds().clipTo(rect);
  ...
  hasAlphaInRect(rect) {
    ...
    var yStride = (this.width - rect.getWidth()) * CPColorBmp.BYTES_PER_PIXEL,
      pixIndex =
        this.offsetOfPixel(rect.left, rect.top) + CPColorBmp.ALPHA_BYTE_OFFSET;
