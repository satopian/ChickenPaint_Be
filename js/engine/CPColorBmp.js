...
CPColorBmp.prototype.edge = function (
  ...
  for (let y = rect.top; y < rect.bottom; y++) {
    for (let x = rect.left; x < rect.right; x++) {
      ...
      if (x > rect.left && y > rect.top) d = Math.min(d, dist[i - w - 1] + 14);
 ...
 */
CPColorBmp.prototype.fillWithNoise = function (rect, color = 0, opacity = 1) {
  rect = this.getBounds().clipTo(rect);
...
CPColorBmp.prototype.getNonTransparentBounds = function (initialBounds) {
  ...
  pixIndex =
    this.offsetOfPixel(result.left, result.top) + CPColorBmp.ALPHA_BYTE_OFFSET;
