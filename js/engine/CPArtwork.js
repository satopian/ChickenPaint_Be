...
export default function CPArtwork(_width, _height) {
  ...
  this.isRemoveLayerAllowed = function () {
    if (curLayer instanceof CPImageLayer) {
      return layersRoot
        ...
        .some((layer) => layer instanceof CPImageLayer && layer != curLayer);
    }
    if (curLayer instanceof CPLayerGroup) {
      return layersRoot
        ...
        .some(
          (layer) =>
            layer instanceof CPImageLayer && !layer.hasAncestor(curLayer),
        );
    }
  ...
  this.setLayerBlendMode = function (blendMode) {
    if (
      ...
      (blendMode != CPBlend.LM_PASSTHROUGH || curLayer instanceof CPLayerGroup)
    ) {
  ...
  this.floodFill = function (
    ...
    const fusion =
      !maskEditingMode && floodFillReferAllLayers ? this.fusionLayers() : null;
  ...
  function CPActionMergeGroup(layerGroup) {
    ...
    let blendTree = new CPBlendTree(layerGroup, that.width, that.height, false),
      blended;
  ...
  class CPActionTransformSelection extends CPUndo {
    constructor() {
      ...
      if (this.movingWholeLayer) {
        ...
        for (
          let i = 0;
          i < this.movingLayers.length && !occupiedSpace.equals(this.srcRect);
          i++
        ) {
          ...
          if (layerInfo.moveImage) {
            occupiedSpace.union(
              layerInfo.layer.image.getNonTransparentBounds(this.srcRect),
            );
          }
        }
      } else {
    ...
    undo() {
      ...
      invalidateLayer(
        ...
        restoreRegions.reduce((a, b) => a.getUnion(b), new CPRect(0, 0, 0, 0)),
        true,
        true,
      );
