    ...
    if (!drawTree) {
      ...
      if (!drawTree) {
        ...
        if (
          ...
          (requireSimpleFusion && (drawTree.alpha < 100 || drawTree.mask))
        ) {
    ...
    for (let i = 0; i < treeNode.layers.length; i++) {
      ...
      if (groupIsEmpty) {
        ...
        fusionHasTransparency =
          fusionHasTransparency && treeNode.image.hasAlphaInRect(blendArea);

        if (DEBUG) {
          console.log(
            `CPBlend.fuseImageOntoImage(treeNode.image, fusionHasTransparency == ${fusionHasTransparency}, childNode.image, childNode.alpha == ${childNode.alpha}, childNode.blendMode == ${childNode.blendMode}, ${blendArea}, ${childNode.mask});`,
          );
        }
    ...
    if (treeNode.clip) {
      ...
      if (baseLayer.alpha < 100) {
        if (baseLayer.mask) {
          if (DEBUG) {
            console.log(
              `CPBlend.replaceAlphaOntoFusionWithTransparentLayerMasked(treeNode.image, baseLayer.image, treeNode.layers[0].alpha == ${treeNode.layers[0].alpha}, ${blendArea});`,
            );
          }
          ...
          if (DEBUG) {
            console.log(
              `CPBlend.replaceAlphaOntoFusionWithTransparentLayer(treeNode.image, baseLayer.image, treeNode.layers[0].alpha == ${treeNode.layers[0].alpha}, ${blendArea});`,
            );
          }
        ...
        if (baseLayer.mask) {
          if (DEBUG) {
            console.log(
              `CPBlend.replaceAlphaOntoFusionWithOpaqueLayerMasked(treeNode.image, baseLayer.image, 100, ${blendArea});`,
            );
          }
