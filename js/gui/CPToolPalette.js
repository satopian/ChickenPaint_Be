...
export default class CPToolPalette extends CPPalette {
  constructor(cpController) {
    super(cpController, "tool", "Tools");
    ...
    function buttonClicked(e) {
      if (this.nodeName == "LI") {
        let button =
          buttons[parseInt(this.getAttribute("data-buttonIndex"), 10)];
    ...
    cpController.on("modeChange", function (newMode) {
      ...
      if (newMode == ChickenPaint.M_DRAW) {
        const toolElem = body.querySelector(
          `li[data-tool="${cpController.getCurTool()}"]`,
        );
        if (toolElem) {
