...
export default function CPCanvas(controller) {
    ...
    CPColorPickerMode.prototype.paint = function () {
      ...
      if (this.shouldPaintBrushPreview) {
        ...
        if (this.curColor) {
          ...
          canvasContext.lineWidth = 22 * window.devicePixelRatio;
          canvasContext.beginPath();
          ...
          canvasContext.lineWidth = 20 * window.devicePixelRatio;
          canvasContext.beginPath();
          canvasContext.arc(cx, cy, radius, 0, Math.PI * 2);
          canvasContext.stroke();
        }
  ...
  function CPTransformMode() {
      ...
      for (let i = 0; i < corners.points.length; i++) {
        if (
          (mouse.x - corners.points[i].x) * (mouse.x - corners.points[i].x) +
            (mouse.y - corners.points[i].y) * (mouse.y - corners.points[i].y) <=
          HANDLE_CAPTURE_RADIUS_SQR
        ) {
          return i * 2;
        }
    ...
    this.mouseDrag = throttle(80, function (e) {
      ...
      if (this.capture) {
        ...
        switch (draggingMode) {
          case DRAG_MOVE:
            let dragPointDoc = roundPoint(coordToDocument(dragPointDisplay)),
  ...
  function handlePointerMoveWrapper(e) {
    ...
    const brushSmall = isAllowedBrush ? controller.getBrushSize() <= 32 : false;
    // 条件に応じて実際の描画関数を呼ぶ
    if (brushSmall) {
