...
export default class CPStrokePalette extends CPPalette {
  constructor(cpController) {
    super(cpController, "stroke", "Stroke");
    ...
    cpController.on("toolChange", function (tool, toolInfo) {
      const freehandElem = document.querySelector(
      ...
      if (freehandElem) {
        freehandElem.classList.toggle(
          "selected",
          toolInfo.strokeMode == CPBrushInfo.STROKE_MODE_FREEHAND,
        );
      }
      if (lineElem) {
        lineElem.classList.toggle(
          "selected",
          toolInfo.strokeMode == CPBrushInfo.STROKE_MODE_LINE,
        );
      }
      if (bezierElem) {
        bezierElem.classList.toggle(
          "selected",
          toolInfo.strokeMode == CPBrushInfo.STROKE_MODE_BEZIER,
        );
      }
    });
  }
