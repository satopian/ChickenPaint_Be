...
let destColor = false,
  destAlpha = false,
  softLightLUTSquare,
  softLightLUTSquareRoot,
  intDiv; // Dummies to silence IDE warnings

const CPRect = function () {}, // Dummy to silence IDE warnings
  BYTES_PER_PIXEL = 4,
  ALPHA_BYTE_OFFSET = 3,
  FUNCTION_MACROS = {
    /**
  ...
  STANDARD_BLEND_OPS = {
    ...
    multiply: {
      ...
      ontoOpaque: function (color1, color2, alpha1) {
        destColor =
          color2 - intDiv((color1 ^ 0xff) * color2 * alpha1, 255 * 255);
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        let newAlpha = (alpha1 + alpha2 - intDiv(alpha1 * alpha2, 255)) | 0,
        ...
        destColor = intDiv(
          color1 * alpha1n2 +
            color2 * alphan12 +
            intDiv(color1 * color2 * alpha12, 255),
          newAlpha,
        );
        destAlpha = newAlpha;
      },
    ...
    multiply2: {
      ...
      ontoOpaque: function (color1, color2, alpha1) {
        destColor =
          color2 - Math.ceil(((color1 ^ 0xff) * color2 * alpha1) / (255 * 255));
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        let newAlpha = (alpha1 + alpha2 - intDiv(alpha1 * alpha2, 255)) | 0,
          alpha12 = intDiv(alpha1 * alpha2, 255),
    ...
    subtract: {
      ...
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        ...
        destColor = intDiv(
          alpha2 * color2 + alpha1 * color1 - alpha12,
          newAlpha,
        );
        destAlpha = newAlpha;
      },
    ...
    lighten: {
      ...
      ontoOpaque: function (color1, color2, alpha1) {
        ...
        destColor =
          color2 >= color1
            ? color2
            : intDiv(color2 * invAlpha1 + color1 * alpha1, 255);
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        let newAlpha = (alpha1 + alpha2 - intDiv(alpha1 * alpha2, 255)) | 0,
        ...
        destColor =
          ((color2 >= color1
            ? color1 * alpha21 + color2 * invAlpha21
            : color2 * alpha12 + color1 * invAlpha12) /
            255) |
          0;
        destAlpha = newAlpha;
      },
    ...
    darken: {
      ...
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        ...
        destColor =
          ((color2 >= color1
            ? color2 * alpha21 + color1 * invAlpha21
            : color1 * alpha12 + color2 * invAlpha12) /
            255) |
          0;
        destAlpha = newAlpha;
      },
        ...
        destColor = intDiv(
          color2 * invAlpha1 +
            alpha1 *
              (color1 == 255
                ? 255
                : Math.min(255, intDiv(255 * color2, color1 ^ 0xff))),
          255,
        );
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        ...
        destColor = intDiv(
          color2 * invAlpha1 +
            alpha1 *
              (color1 == 0
                ? 0
                : Math.min(255, (255 * (color2 ^ 0xff)) / color1) ^ 0xff),
          255,
        );
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
        ...
        destColor = intDiv(
          invAlpha1 * color2 +
            (color2 <= 127
              ...
              : alpha1 *
                (((2 * (color1 ^ 0xff) * (color2 ^ 0xff)) / 255) ^ 0xff)),
          255,
        );
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
      ...
      ontoOpaque: function (color1, color2, alpha1) {
        ...
        destColor = intDiv(
          invAlpha1 * color2 +
            alpha1 *
              (color1 <= 127
                ? intDiv((2 * color1 - 255) * softLightLUTSquare[color2], 255) +
                  color2
                : intDiv(
                    (2 * color1 - 255) * softLightLUTSquareRoot[color2],
                    255,
                  ) + color2),
          255,
        );
            ...
            (color1 <= 127
              ? alpha1 *
                (color1 == 0
                  ? 0
                  : 255 -
                    Math.min(255, intDiv((255 - color2) * 255, 2 * color1)))
              : alpha1 *
                (color1 == 255
                  ? 255
                  : Math.min(255, intDiv(color2 * 255, 2 * (255 - color1))))),
          255,
        );
      },
      ontoTransparent: function (color1, color2, alpha1, alpha2) {
  ...
  PASSTHROUGH_OPERATION = {
    ...
    ontoOpaque: function (color1, color2, alpha1, alphaMix, invAlphaMix) {
      let realAlpha = alpha1 * alphaMix + 255 * invAlphaMix;

      destColor = intDiv(
        color1 * alpha1 * alphaMix + color2 * 255 * invAlphaMix,
        realAlpha,
      );
      destAlpha = realAlpha;
    },

    ontoTransparent: function (
      color1,
      color2,
      alpha1,
      alpha2,
      ...
      destColor = intDiv(
        color1 * alpha1 * alphaMix + color2 * alpha2 * invAlphaMix,
        realAlpha,
      );
      destAlpha = realAlpha;
    },
  ...
  UPGRADE_MULTIPLY_OPERATION = {
    ...
    ontoOpaque: function (color1, color2, alpha1) {
      ...
      destColor =
        color1 +
        Math.ceil(
          (((255 - color1) * color2 * alpha1) % (255 * 255)) /
            (color2 * alpha1),
        );
    },
  };
...
function getLayerAlphaExpressionForVariant(
  alphaExpression,
  operation,
  variant,
) {
  let rounding = variant.unroundedAlpha ? "" : " | 0";
...
function applyFunctionMacro(code, macroName, macroFunc) {
  ...
  while ((macroStartPos = code.indexOf(macroName)) !== -1) {
    let // Search for the parenthesised argument list after the name of the macro
      ...
      finishArgument = function () {
        ...
        macroArguments.push(
          codeToSearch
            .slice(argumentStartPos, splittingChars.lastIndex - 1)
            .trim(),
        );
      ...
      finishMacro = function () {
        let macroEndPos = splittingChars.lastIndex,
          macroResult = macroFunc.apply(null, macroArguments);

        code =
          code.slice(0, macroStartPos) +
          macroResult +
          codeToSearch.slice(macroEndPos);
      };
...
function applyVectorAssignmentSubstitutions(
  code,
  useColor1Var,
  useColor2Var,
  destPixIndexVar,
   ...
   */
  code = code.replace(
    /^\s*destColor\s*=\s*([^;]+)\s*;/gm,
    function (match, newValueExpr) {
      let vectorCode = "";

      for (let i = 0; i < 3; i++) {
        let addI = i ? " + " + i : "",
          destPixelExpr = destinationIsLayer
            ? `layer.data[${destPixIndexVar}${addI}]`
            : `fusion.data[${destPixIndexVar}${addI}]`;
...
function getFunctionBody(func) {
  let source = Function.prototype.toString.call(func);
...
function leftAlign(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].match(/\S/)) {
      let indent = lines[i].match(/^\t*/)[0].length;
  ...
  if (minIndent > 0) {
    let leadingTabs = new RegExp("^\t{" + minIndent + "}");
...
function tabIndentLines(text, indent, includeFirstLine) {
  let lines = leftAlign(text).split("\n");
...
function makeBlendOperation(functionName, operation, variant) {
  ...
  innerVars = {
    alpha1: getLayerAlphaExpressionForVariant(
      "layer.data[pixIndex + ALPHA_BYTE_OFFSET]",
      operation,
      variant,
    ),
  };
  ...
  if (operation.customAlphaMix) {
    innerVars.alpha2 = getFusionAlphaExpressionForVariant(
      `fusion.data[${destPixIndexVar} + ALPHA_BYTE_OFFSET]`,
      variant,
    );
  ...
  kernel = applyVectorAssignmentSubstitutions(
    kernel,
    useColor1Var,
    useColor2Var,
    destPixIndexVar,
    operation.destinationIsLayer,
  );
  kernel = applyMacros(kernel);

  let yIncrements = ["y++", "pixIndex += yStride"],
...
function makeBlendOperations() {
  ...
  for (let opName in STANDARD_BLEND_OPS) {
    let operation = STANDARD_BLEND_OPS[opName];
    ...
    for (let variant of OPERATION_VARIANTS) {
      let functionName = opName + "Onto" + capitalizeFirst(variant.name);

      if (
        (variant.fusionHasTransparency && operation.ontoTransparent) ||
        (!variant.fusionHasTransparency && operation.ontoOpaque)
      ) {
        functionStrings.push(
          makeBlendOperation(functionName, operation, variant),
        );
      }
    }
  }
...
${makeBlendOperation(
  "passthroughOntoTransparentFusionWithTransparentLayerMasked",
  PASSTHROUGH_OPERATION,
  {
    fusionHasTransparency: true,
    layerAlpha100: false,
...
${makeBlendOperation(
  "replaceOntoFusionWithTransparentLayer",
  REPLACE_OPERATION,
  {
    fusionHasTransparency: false,
    layerAlpha100: false,
...
${makeBlendOperation(
  "replaceOntoFusionWithTransparentLayerMasked",
  REPLACE_OPERATION,
  {
    fusionHasTransparency: false,
    layerAlpha100: false,
...
${makeBlendOperation(
  "replaceOntoFusionWithOpaqueLayerMasked",
  REPLACE_OPERATION,
  {
    layerAlpha100: true,
    masked: true,
...
${makeBlendOperation(
  "replaceAlphaOntoFusionWithTransparentLayerMasked",
  REPLACE_ALPHA_OPERATION,
  {
    layerAlpha100: false,
    masked: true,
...
${makeBlendOperation(
  "replaceAlphaOntoFusionWithOpaqueLayer",
  REPLACE_ALPHA_OPERATION,
  {
    layerAlpha100: true,
  },
...
${makeBlendOperation(
  "replaceAlphaOntoFusionWithOpaqueLayerMasked",
  REPLACE_ALPHA_OPERATION,
  {
    layerAlpha100: true,
    masked: true,
...
${makeBlendOperation(
  "upgradeMultiplyOfTransparentLayer",
  UPGRADE_MULTIPLY_OPERATION,
  {
    layerAlpha100: false,
    fusionHasTransparency: false,
...
${makeBlendOperation(
  "_normalFuseImageOntoImageAtPosition",
  STANDARD_BLEND_OPS.normal,
  {
    layerAlpha100: true,
